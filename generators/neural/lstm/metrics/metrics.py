...
def calculate_note_difference(note_name1, note_name2):
    n1 = music21.note.Note(note_name1)
    n2 = music21.note.Note(note_name2)
    interval_semitones = music21.interval.notesToInterval(n1, n2).semitones
...
def calculate_chord_similarity(chord_a, chord_b):
    ...
    for i in range(len(smaller_chord)):
        for note in larger_chord:
            similarity = 1 - \
                (calculate_note_difference(smaller_chord[i], note) / 24)
...
def compare_elements(element1, element2):
    if ('.' in element1) and ('.' in element2):
        ...
        return 1 - \
            (calculate_chord_similarity(chord1_notes, chord2_notes) / 24)
...
def calculate_similarity(notes_path, model_path, N, index):
    ...
    expected_notes = \
        [int_to_note[k] for k in input_sequences[index + SEQUENCE_LENGTH]]
    ...
    for i in range(N):
        ...
        expected = int_to_note[output_sequences[index + i + SEQUENCE_LENGTH]]
        expected_notes.append(expected)
        ...
        similarity = \
            calculate_noteslist_similarity(expected_notes, predict_notes)
